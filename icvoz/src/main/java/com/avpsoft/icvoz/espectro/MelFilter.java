 ...
 *	edu.cmu.sphinx.frontend.frequencywarp.MelFilter.java
 *
...
/**
 * Defines a triangular mel-filter. The
 * {@link edu.cmu.sphinx.frontend.frequencywarp.MelFrequencyFilterBank} creates
 * mel-filters and filters spectrum data using the method
 ...
 * are given to the {@link MelFilter Constructor}. The first three arguments to
...
public class MelFilter {
     ...
     * frequency, so we can determine the right and left slopes, which could be
     * not only assymmetric but completely different. We're also given the
     * initial frequency, which may or may not coincide with the left edge, and
     * the frequency step.
     *
    ...
    public MelFilter(double leftEdge,
         ...
         * <code>weight</code> field by computing how many frequency bins we can
         * fit in the current frequency range.
         */
        ...
        for (currentFreq = initialFreq, indexFilterWeight = 0;
            ...
            /**
             * A straight line that contains point <b>(x0, y0)</b> and has slope
             * <b>m</b> is defined by:
             *
             * <b>y = y0 + m * (x - x0)</b>
             *
    ...
    /**
     * Compute the output of a filter. We're given a power spectrum, to which we
     * apply the appropriate weights.
     *
     * @param spectrum the input power spectrum to be filtered
     *
     * @return the filtered value, in fact a weighted average of power in the
     * frequency range of the filter pass band
     */
