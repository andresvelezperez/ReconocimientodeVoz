/*
 *	Paquete				: rna
 *	Archivo				: Network.java
 *	Version                         : 1.0.0		2007-04-16
 *	Autor				: Jeff Heaton
 *	URL				: http://www.jeffheaton.com
 *	Copyright			: Jeff Heaton, GNU public license (LGPL).
 *	Version				: 1.0 2002
 *
 *	Este codigo esta basado en una implementacion libre de una
 *	Red Neuronal realizado por Jeff Heaton. Disponible para descarga
 *	desde "solocodigo" (http://www.solocodigo.com) y el foro de este
 *	(http://foros.solocodigo.com/index.php?showtopic=17698) con el
 *	enlace a (http://foros.solocodigo.com/index.php?s=cd436757af7c825d3bf70fce9d67e57d&act=Attach&type=post&id=58839)
 *
...
public class Network {
     ...
     *
     * @param input entrada de la red neuronal
     * @param output la salida que debe retornar la red dada una entrada.
     */
    public void training(double input[][], double output[][]) {
        if (input.length != output.length) {
            throw new java.lang.IndexOutOfBoundsException("Network: Length input != Length output");
        }
    ...
    public void reset(double[] pesos, double[] umbral) {
        int i;

        for (i = 0; i < neuronCount; i++) {
            thresholds[i] = umbral[i];
            thresholdDelta[i] = 0;
            accThresholdDelta[i] = 0;
        }
        for (i = 0; i < matrix.length; i++) {
